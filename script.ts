...
class AnswerListener {
    ...
    static #Result(ResultScore: number) {
        ...
        let quizz = document.getElementById('Q');
        if (quizz) {
            if (ResultScore <= 4) quizz.innerHTML = `<h3>Tu as eu ${ResultScore}/10<br>Tu me connais pas du tout</h3>`;
            else if (ResultScore <= 9) quizz.innerHTML = `<h3>Tu as eu ${ResultScore}/10<br>Bien joué tu me connais presque parfaitement</h3>`;
            else quizz.innerHTML = `<h3>Tu as eu ${ResultScore}/10<br>Incroyable! tu me connais aussi bien que moi je connais ma vie!</h3>`;
        }
    }
}
